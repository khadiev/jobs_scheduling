...
class Hours:
    ...
    def __init__(self):
        ...
        self.minutes_in_year = 365 * self.minutes_in_24h
        self.minutes_in_year_up = 1000000
    ...
    def get_sanctioned_breakdown(self, start_minute, duration):
        ...
        full_days = int(math.floor(duration / (self.minutes_in_24h)))

        sanctioned = full_days * self.hours_per_day * 60
        unsanctioned = full_days * (24 - self.hours_per_day) * 60
        remainder_start = round(start_minute + full_days * self.minutes_in_24h)

        for minute in range(remainder_start, start_minute+duration):
    ...
    def next_sanctioned_minute(self, minute):
        """ Given a minute, finds the next sanroundctioned minute.
        ...
        if self.is_sanctioned_time(minute) and self.is_sanctioned_time(minute+1):
            return minute + 1
        num_days = int(math.floor(minute / self.minutes_in_24h))
    ...
    def apply_resting_period(self, start, num_unsanctioned):
        ...
        """
        num_days_since_jan1 = int(math.floor(start / self.minutes_in_24h))
        rest_time = num_unsanctioned
        rest_time_in_working_days = int(math.floor(rest_time / (60 * self.hours_per_day)))
    ...
    def get_day_begin_minute(self, elf_available_time, day_begin_minute):
        while not(day_begin_minute<=elf_available_time and day_begin_minute+24*60>elf_available_time):
            day_begin_minute += 24*60
        return day_begin_minute